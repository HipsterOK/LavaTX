...
void backlightEnable(uint8_t level)
{
  ...
  if (level == 0) {
    ...
    GPIO_ResetBits(BACKLIGHT_GPIO, BACKLIGHT_GPIO_PIN); // ВКЛ
  }
}

uint8_t isBacklightEnabled(void)
{
  // Для простого GPIO управления всегда возвращаем true
  // Реальная проверка состояния подсветки не поддерживается
  return true;
}
