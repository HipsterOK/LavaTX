...
{
  TRACE("runStartupAnimation: starting");
 ...
 void opentxInit()
 {
  ...
  TRACE("TBS startup: watchdog=%d, storage_erase=%d, board_off=%d, sd_present=%d",
        WAS_RESET_BY_WATCHDOG() ? 1 : 0,
        bkregGetStatusFlag(STORAGE_ERASE_STATUS) ? 1 : 0,
        getBoardOffState() ? 1 : 0,
        SD_CARD_PRESENT() ? 1 : 0);

  // ИСПРАВЛЕНИЕ: Для TBS Tango всегда показывать анимацию запуска,
  // независимо от состояния флагов, когда SD карта отсутствует
  if (SD_CARD_PRESENT() && (WAS_RESET_BY_WATCHDOG() || bkregGetStatusFlag(STORAGE_ERASE_STATUS) || getBoardOffState())) {
    TRACE("TBS startup: SD present, special case - skipping animation, direct power on");
    ...
    TRACE("TBS startup: running startup animation");
    runStartupAnimation();
  }
